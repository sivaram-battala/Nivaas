...
axiosStore.interceptors.request.use(async function (config) {
  let token = await getBroadLeafAuthTokenDetails();
  console.log('Sending req with this braodleaf auth token ===>', token);
...
axiosNewData1.interceptors.request.use(async function (config) {
  let token = await getAuthTokenDetails();
  // console.log('token basic auth of fanfun apis===>>', token);
  config.headers.Authorization = token;
  return config;
});
